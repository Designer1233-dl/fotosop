...
# Premiumize outgoing HTML text centrally so we do not miss scattered literals.
...
def empty_treasury_user_text() -> str:
    return (
        f"{premium_emoji('withdraw', '💸')} <b>Казна пуста</b>\n\n"
        "Недостаточно средств на резерве для выплаты.\n"
        "Казну скоро пополнят, попробуйте чуть позже."
    )
...
def bets_menu_keyboard() -> InlineKeyboardMarkup:
    ...
    builder.button(
        text="Куб 65 x7 • 6 и 5",
        callback_data="bet:sixty_five",
        icon_custom_emoji_id=premium_button_icon("bet_sixty_five_button"),
    )
...
def profile_keyboard() -> InlineKeyboardMarkup:
    ...
    builder.button(text="История", callback_data="profile:history", icon_custom_emoji_id=premium_button_icon("history_button"))
...
def format_transaction_label(tx_type: str) -> str:
    mapping = {
        "deposit": "Пополнение",
        "withdraw": "Вывод",
        ...
        "room_hold": "Вход в комнату",
        "room_refund": "Возврат из комнаты",
        "room_join": "Присоединение к комнате",
        "room_win": "Победа в комнате",
        "referral": "Реферальное начисление",
        "promo": "Промокод",
        "gift_check": "Чек",
    }
    return mapping.get(tx_type, tx_type.replace("_", " ").strip().title())


async def render_user_history(user_id: int) -> str:
    user = await db.get_user(user_id)
    if not user:
        return f"{premium_emoji('history_title', '🧾')} <b>История операций</b>\n\nПрофиль не найден."

    rows = await db.get_user_transactions(user_id, limit=10)
    if not rows:
        return f"{premium_emoji('history_title', '🧾')} <b>История операций</b>\n\nОпераций пока нет."

    lines: list[str] = [f"{premium_emoji('history_title', '🧾')} <b>История операций</b>", ""]
    for row in rows:
        amount = float(row["amount"])
        sign = "+" if amount >= 0 else "-"
        label = format_transaction_label(str(row["type"]))
        created_at = str(row["created_at"]).replace("T", " ")[:16]
        meta = str(row["meta"] or "").strip()
        line = (
            f"{premium_emoji('history_entry', '•')} {label}: "
            f"<b>{sign}{fmt_amount(abs(amount))} {config.bot_asset}</b> "
            f"<code>{html.escape(created_at)}</code>"
        )
        lines.append(line)
        if meta:
            lines.append(f"└ {html.escape(meta[:80])}")
    return "\n".join(lines)
...
async def show_bets_menu(target: Message | CallbackQuery) -> None:
    ...
    text = (
        ...
        f"{premium_emoji('dice_button', '🎲')} <b>Куб 65 x7</b> — первый кубик должен выпасть на <b>6</b>, второй на <b>5</b>.\n"
...
def withdraw_mode_keyboard(auto_enabled: bool) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    if auto_enabled:
        builder.button(text="Отключить авто-вывод", callback_data="withdrawmode:toggle", icon_custom_emoji_id=premium_button_icon("auto_withdraw_off"))
    else:
        builder.button(text="Включить авто-вывод", callback_data="withdrawmode:toggle", icon_custom_emoji_id=premium_button_icon("auto_withdraw_on"))
    builder.button(text="Назад", callback_data="admin:open", icon_custom_emoji_id=premium_button_icon("back_button"))
    builder.adjust(1)
    return builder.as_markup()
...
@router.callback_query(F.data == "bet:sixty_five")
async def cb_bet_sixty_five(callback: CallbackQuery, state: FSMContext) -> None:
    minimum = await get_min_bet_amount()
    await state.set_state(UserStates.bet_sixty_five_amount)
    await callback.message.answer(
        f"{premium_emoji('bets_button', '🎯')} <b>Куб 65 x7</b>\n\n"
        "Победа, если первый кубик выпадет на <b>6</b>, а второй на <b>5</b>.\n"
        f"Минимальная ставка: <b>{fmt_amount(minimum)} {config.bot_asset}</b>\n"
        "Отправьте сумму одним сообщением."
    )
    await callback.answer()
...
async def cb_profile_history(callback: CallbackQuery) -> None:
    text = await render_user_history(callback.from_user.id)
    await safe_edit(callback.message, text, reply_markup=profile_keyboard())
    await callback.answer()
    ...
    try:
        ...
        if is_empty_treasury_error(str(exc)):
            await send_log(
                callback.bot,
                "🚨 <b>Казна пуста</b>\n"
                "CryptoBot не смог создать чек для одобрения вывода из-за нехватки средств на резерве.\n\n"
                f"👤 Пользователь: <code>{withdrawal['user_id']}</code>\n"
...
async def state_withdraw_amount(message: Message, state: FSMContext) -> None:
    ...
    if auto_enabled == "1":
        ...
        try:
            ...
            if is_empty_treasury_error(str(exc)):
                await message.answer(empty_treasury_user_text())
                await send_log(
                    message.bot,
                    "🚨 <b>Казна пуста</b>\n"
                    "CryptoBot не смог создать чек из-за нехватки средств на резерве.\n\n"
                    f"👤 Пользователь: <code>{message.from_user.id}</code>\n"
...
@router.message(UserStates.bet_sixty_five_amount)
async def state_bet_sixty_five_amount(message: Message, state: FSMContext) -> None:
    await process_stake_bet(
        message,
        state,
        minimum=await get_min_bet_amount(),
        ...
        multiplier=7.0,
        tx_code="sixty_five",
        win_checker=lambda first, second: first == 6 and second == 5,
        details_builder=lambda first, second: f"{first} и {second} — точное попадание 6 и 5" if first == 6 and second == 5 else f"{first} и {second} — нужно 6 и 5 по порядку",
    )
